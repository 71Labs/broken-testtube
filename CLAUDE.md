...
- `src/app/page.tsx` — composes the landing page from section components in order: Header → Hero → Talise → Features → Gallery → Dashboard → Capabilities → ContactCTA → Footer.
- `src/app/globals.css` — Tailwind import + the design system (see below).
- `src/components/site/` — one file per landing section. Server Components by default; `header.tsx`, `reveal.tsx`, and `dashboard.tsx` are `"use client"` (scroll state, IntersectionObserver, interactive filter tabs).
- **`dashboard.tsx`** — a "Talise for web · Preview" showcase: a browser-framed dashboard mock (sidebar nav, insight cards with an SVG balance sparkline, interactive filter tabs, and a records table with type chips + status pills), built in the style of the *Beautiful UI* suite (beautifului.dev) adapted to payments. Labelled "Preview" because Talise's real product is mobile-only; keep it framed as a concept, not a shipped web app. **Content note:** 71Labs is the studio; **Talise** (talise.io — a consumer stablecoin payments app on Sui) is its one real, live product. `talise.tsx` (spotlight), `features.tsx`, and `gallery.tsx` are built from Talise's actual value props and **real app screenshots in `public/talise/`** — keep copy truthful, don't invent additional products or investors.
