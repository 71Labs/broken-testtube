...
- **Schema**: `profiles` (role: admin|worker), `projects`, `tasks` (status todo|in_progress|done, priority, project/assignee FKs), `activity`. RLS: everyone reads; workers update only their assigned tasks; admins manage everything; only admins write projects/roles.
- **Routes**: `panel/login` (email/password, sign in/up) outside the shell; the authed app lives in the `panel/(app)/` route group with its own sidebar layout — `page.tsx` (overview), `tasks`, `projects`, `team`, `profile`. Data fetchers in `src/lib/panel/data.ts`; mutations are server actions in `src/app/panel/actions.ts` (they log to `activity` and `revalidatePath`). Shared UI in `panel/_components/`. Types in `src/lib/panel/types.ts`.
