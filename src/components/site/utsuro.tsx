...
function UtsuroPreview() {
  return (
    <div className="rounded-3xl border border-border bg-[#f6f5f2] p-6">
      ...
      <div className="overflow-hidden rounded-2xl border border-neutral-200 bg-white shadow-[0_30px_80px_-30px_rgba(0,0,0,0.25)]">
        <div className="flex items-center gap-3 border-b border-neutral-200 px-4 py-2.5">
          ...
          <div className="mx-auto rounded-md border border-neutral-200 bg-neutral-50 px-3 py-1 font-mono text-[10px] text-neutral-400">
            utsuro.xyz/home
          </div>
        </div>
        <Image src="/utsuro/app.png" alt="Utsuro app" width={3456} height={1964} sizes="(min-width:1024px) 520px, 100vw" className="h-auto w-full" />
      </div>
      {/* render strip */}
      <div className="mt-3 grid grid-cols-4 gap-2">
        {RENDERS.map((r) => (
          <Image key={r} src={`/utsuro/${r}.png`} alt="Utsuro render" width={378} height={378} sizes="120px" className="aspect-square w-full rounded-lg object-cover" />
        ))}
      </div>
    </div>
  );
